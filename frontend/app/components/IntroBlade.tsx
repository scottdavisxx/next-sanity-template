...
export default function Intro({ titles, blurb, ctas, bgImage }: IntroProps) {
    ...
    return (
        <div className="relative flex flex-col px-24 py-8 gap-2  justify-between min-h-[478px]">
            ...
            <div className="relative flex flex-col md:flex-row px-6 py-8 gap-8 md:justify-between items-center z-10">
                {
                    titles.map((obj) => {
                        return <h2 key={obj.title} className="text-4xl font-bold w-full md:w-1/3 text-center">{obj.title}</h2>
                    })
                }
            </div>
            <div className="relative flex px-6 py-8 gap-2 justify-between items-center z-10">
